...
<div>
    <div style="margin-bottom: 20p;">
        <div v-if="!mesaSeleccionada">
            ...
            <div v-for="(mesa, idx) in mesas" :key="idx" style="border: 1px solid; margin: 30px; border-radius: 5px; padding: 10px; cursor: pointer;">
                <h3>
                    <strong>{{ mesa.nombre }}</strong>
                </h3>
    
                <p @click="mesaSeleccionada = mesa">
        ...
        <div v-if="mesaSeleccionada" style="display: flex;">
            <div>
                ...
                <div>
                    <h3>
                        <strong> Selecciono la mesa : {{ mesaSeleccionada.nombre }}</strong>
                    </h3>
                </div>
    ...
    <div v-if="pedidos.length > 0">
        ...
        <p>
            <span @click="cargarPedidos" style="cursor: pointer;">Cargar</span>
        </p>

        <div v-for="(pedido, idx) in pedidos" :key="idx" style="border: 1px solid; margin: 30px; border-radius: 5px; padding: 10px; cursor: pointer;">
            <div v-if="obtenerMesaSeleccionada(pedido)">
                Mesa: {{ obtenerMesaSeleccionada(pedido).nombre }}
            </div>
            <div v-if="obtenerMenuSeleccionado(pedido)">
                Pedido: {{ obtenerMenuSeleccionado(pedido).nombre }} ({{ obtenerMenuSeleccionado(pedido).precio }})
            </div>
            <div v-if="pedido.nombreCliente">
                <p>Nombre Cliente: {{ pedido.nombreCliente }}</p>
            </div>
            <div v-if="pedido.direccionCliente">
                <p>Direccion Cliente: {{ pedido.direccionCliente }}</p>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        cargarPedidos () {
            getDocs(collection(db, 'dbPedido'))
            .then((querySnapshot) => {
                const pedidos = [];
